...
function authDebugJsonFragmentEnd(text, start) {
	const stack = [];
	let inString = false;
	let escaped = false;
...
function authDebugSanitizeJsonFragments(value) {
	...
	while (offset < text.length) {
		...
		const end = authDebugJsonFragmentEnd(text, start);
		if (end === -1) {
			sanitized += text.slice(offset);
			break;
		}
		const fragment = text.slice(start, end + 1);
		sanitized += text.slice(offset, start) + (authDebugSanitizeJsonLine(fragment) || fragment);
		offset = end + 1;
	}
	return sanitized;
}
